package consulting.emrich.c64watch
...
class MyWatchFace : CanvasWatchFaceService() {
    ...
    inner class Engine : CanvasWatchFaceService.Engine() {
        ...
        private var mDarkBlue: Int = 0
        private var mLightBlue: Int = 0
        private lateinit var mC64Font: Typeface
        private lateinit var mBGPaint: Paint
        private lateinit var mFGTimePaint: Paint
        private lateinit var mFGDayPaint: Paint
        ...
        private fun initializeBackground() {
            ...
            mDarkBlue = Color.parseColor("#0139ff")
            mLightBlue = Color.parseColor("#4e7fff")
            mC64Font = Typeface.createFromAsset(resources.getAssets(), "fonts/c64.ttf")
            mBGPaint = Paint()
            mBGPaint.color = mDarkBlue
            mFGTimePaint = Paint()
            mFGTimePaint.color = mLightBlue
            mFGTimePaint.typeface = mC64Font
            mFGTimePaint.textSize = 80f
            mFGTimePaint.textAlign = Paint.Align.CENTER
            mFGDayPaint = Paint()
            mFGDayPaint.color = mLightBlue
            mFGDayPaint.typeface = mC64Font
            mFGDayPaint.textSize = 32f
            mFGDayPaint.textAlign = Paint.Align.CENTER
        }
        ...
        private fun drawWatchFace(canvas: Canvas) {
            val hour24 = mCalendar[Calendar.HOUR_OF_DAY]
            val minute = mCalendar[Calendar.MINUTE]
            val second = mCalendar[Calendar.SECOND]
            val millis = mCalendar[Calendar.MILLISECOND]
            val dayOfWeek = mCalendar[Calendar.DAY_OF_WEEK]
            val dayOfMonth = mCalendar[Calendar.DAY_OF_MONTH]
            val isPM = mCalendar[Calendar.AM_PM] == Calendar.PM

            val TimeString = String.format(Locale.getDefault(), "%02d:%02d", hour24, minute)
            val DayString = String.format("%1\$tA,", mCalendar)
            val DateString = String.format("%1\$tb %1\$te, %1\$tY", mCalendar)
            ...
            if (batteryStatus != null)
            {
                ...
                if (batteryScale > 0)
                {
                    BatteryString = String.format("%d%%", Math.round(batteryLevel * 100f / batteryScale))
                }
            }


            canvas.drawText(BatteryString, mCenterX, mCenterY - 120f, mFGDayPaint)
            canvas.drawText(TimeString, mCenterX, mCenterY, mFGTimePaint)
            canvas.drawText(DayString, mCenterX, mCenterY + 40f, mFGDayPaint)
            canvas.drawText(DateString, mCenterX, mCenterY + 80f, mFGDayPaint)
            canvas.drawText("READY.", mCenterX, mCenterY + 144f, mFGDayPaint)
        }
